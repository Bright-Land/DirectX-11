...
{
	VertexType_ *vertices, *texcoords, *normals;
	FaceType *faces;
	ifstream fin;
	...
	vertices = new VertexType_[vertexCount];
	if (!vertices)
	{
		return false;
	}

	texcoords = new VertexType_[textureCount];
	if (!texcoords)
	{
		return false;
	}

	normals = new VertexType_[normalCount];
	if (!normals)
	{
		return false;
	}
	...
	fout.open("../Engine/data/MadeModel.txt");
...
// 여기서 받는 modelFilename은 obj파일을 가리킨다.
